    ...
    fs.readdir(downloadPath, function(err, files) {
      ...
      async.each(newReleases, function(release, cb) {
        ...
        gethub('jordonias', 'game_client', release, releasePath, function(err) {
          var duo = spawn('duo', ['game.js'], { cwd: releasePath });

          duo.stdout.on('data', function(data) {});

          duo.stderr.on('data', function(data) {
            return cb('Failed to build ' + release + '.');
          });

          duo.on('close', function(code) {
            return cb();
          });
        });
      }, function(err) {
